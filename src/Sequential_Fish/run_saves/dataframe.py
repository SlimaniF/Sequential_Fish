...
def check_run_dataframe(run_dataframe: pd.DataFrame) :
    ...
    if len(run_dataframe) != len(run_dataframe['run_id'].unique()) or cast(bool,run_dataframe['run_id'].isna().any()) : 
...
def create_new_run(index=None) -> pd.DataFrame:
    ...
    if index is None :
        if len(run_dataframe) == 0 :
            ...
            max_index = cast(int, run_dataframe.index.max())
            index = pd.Index([max_index + 1])
    else :
...
def _get_run_path_index(run_dataframe : pd.DataFrame, run_path : str) -> pd.Index | None :
...
def get_parameter(
    ...
    parameters = run_dataframe.at[index, parameter.upper()]
