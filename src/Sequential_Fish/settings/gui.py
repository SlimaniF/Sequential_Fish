    ...
    def get_parameters(self) -> Dict[str, Any]:
        ...
        for name, default in self._param_types.items():
            ...
            # Dict
                ...
                inside_type = get_args(att_type)[1]
                d: Dict[str, Any] = {}
                if not txt == "" :
                    default_value = self._default_value[name].values() if not self._default_value[name] is None else {}.values()
                    sample_v = next(iter(default_value))
