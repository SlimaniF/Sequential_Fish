...
with tifffile.TiffFile(file_path_other) as tif :
    ...
    print(f"\nFILENAME : {file_path_other}")
    cycle_metadata = ome_types.from_xml(tif.pages[0].description)
    print("images len : ", len(cycle_metadata.images))
    if len(cycle_metadata.images) > 0 :
        print("Image[0] : ", dict(cycle_metadata.images[0]).keys())
        print("Image[0] pixels datablocks : ", cycle_metadata.images[0].pixels.tiff_data_blocks)
        print("Image[0] pixels keys : ", dict(cycle_metadata.images[0].pixels).keys())
        print("Image[0] size_t : ", cycle_metadata.images[0].pixels.size_t)
        print("Image[0] size_z : ", cycle_metadata.images[0].pixels.size_z)
        print("Image[0] size_c : ", cycle_metadata.images[0].pixels.size_c)
        print("Image[0] size_y : ", cycle_metadata.images[0].pixels.size_y)
        print("Image[0] size_x : ", cycle_metadata.images[0].pixels.size_x)
        print("Image[0] dimension_order : ", cycle_metadata.images[0].pixels.dimension_order)
        print("Image[0] physical size z : ", cycle_metadata.images[0].pixels.physical_size_z, cycle_metadata.images[0].pixels.physical_size_z_unit)
        print("Image[0] physical size y : ", cycle_metadata.images[0].pixels.physical_size_y, cycle_metadata.images[0].pixels.physical_size_y_unit)
        print("Image[0] physical size x : ", cycle_metadata.images[0].pixels.physical_size_x, cycle_metadata.images[0].pixels.physical_size_x_unit)
    else :
        print(dict(cycle_metadata))

with tifffile.TiffFile(file_path_main) as tif :
    metadata = ome_types.from_xml(tif.pages[0].description)
    main_uuid = metadata.uuid
    print("main_uuid : ", main_uuid)
    ...
    print("raw shape : ", image_stack.shape)
...
print("binary only : " , metadata.binary_only)
print("uuid : ", metadata.uuid)




if cycle_number < TARGETED_CYCLE_NUMBER :
    print(f"found {cycle_number} cycles. Adding an empty cycles to reach {TARGETED_CYCLE_NUMBER} cycles.")
    ...
    with tifffile.TiffWriter(os.path.join(output_path, new_filename), bigtiff=True) as tif :
        ...
        cycle_metadata.uuid = tiff_uuid
        metadata.images[0].pixels.tiff_data_blocks.append(
            TiffData(
                first_t = cycle_number, #No +1 bc starts at 0
                plane_count = len(empty_cycle),
                uuid=TiffData.UUID(value=tiff_uuid, file_name=new_filename)
            )
        )
        metadata.images[0].pixels.size_t += 1
        ...
        tif.write(
                ...
                dtype=dtype,
                description=ome_types.to_xml(cycle_metadata),
                compression=compression
            )

    with tifffile.TiffWriter(os.path.join(output_path, filename), bigtiff=True, shaped=False) as tif:
        print(f"Writing main tiff of shape : {image_stack[0].shape} with order : {main_axes_order}\nwith uuid : {metadata.uuid}")
        tif.write(
            image_stack[0],
...
print("Opening resulting image : ")
with tifffile.TiffFile(os.path.join(output_path, filename)) as tif :
...
with tifffile.TiffFile(os.path.join(output_path, new_filename)) as tif :
